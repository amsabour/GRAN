...
class GraphStar(nn.Module):
    ...
    def forward(self, x, edge_index, batch, star=None, y=None, edge_attr=None, edge_type=None):
        # times = []
        if self.training:
            tw.train_steps += 1
        else:
        ...
        edge_index, edge_type, edge_attr = self.add_self_loop_edge(edge_index, edge_type, edge_attr, num_node)

        if self.num_star > 0:
            edge_index, edge_type, edge_attr = self.add_star_edge(edge_index, edge_type, edge_attr, num_node, batch)
        ...
        for conv, star_attn, i in zip(self.conv_list, self.star_attn_list, range(len(self.conv_list))):
            # update node
            x = conv(x, stars.view(-1, self.hid), None, edge_index, edge_type=edge_type, edge_attr=edge_attr)
            # update star
    ...
    def add_star_edge(self, edge_index, edge_type, edge_attr, x_size, batch):
        ...
        for i in range(self.num_star):
            ...
            if edge_attr is not None:
                edge_attr = torch.cat([edge_attr, edge_attr.new_full((x_size,), 1)])

        return edge_index, edge_type, edge_attr

    def add_self_loop_edge(self, edge_index, edge_type, edge_attr, x_size):
        dtype, device = edge_index.dtype, edge_index.device
        ...
        if edge_attr is not None:
            edge_attr = torch.cat([edge_attr, edge_attr.new_full((x_size,), 1)], dim=0)
