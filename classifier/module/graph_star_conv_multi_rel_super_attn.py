...
class GraphStarConv(MessagePassing):
    ...
    def forward(self, x, stars, init_x, edge_index, edge_attr=None, edge_type=None):
        """"""
        num_node = x.size(0)
        nodes = x
        ...
        nodes = self.propagate('add', edge_index, edge_type, edge_attr=edge_attr, x=nodes, num_nodes=num_node)

        return nodes

    def propagate(self, aggr, edge_index, edge_type, edge_attr=None, size=None, **kwargs):
        ...
        for i in range(self.num_relations):
            ...
            if edge_index_mask.sum() > 0:
                ...
                edge_weights = edge_attr[edge_index_mask]
                ...
                score = self.cal_att_score(xq, xk, self.heads, weights=edge_weights)
