...
class DiffPool(nn.Module):
    ...
    def __init__(self, dim_features, dim_target, max_num_nodes, num_layers=2, gnn_dim_hidden=64, dim_embedding=128,
                 dim_embedding_MLP=50):
        super().__init__()

        self.max_num_nodes = max_num_nodes
        num_diffpool_layers = num_layers
        gnn_dim_hidden = gnn_dim_hidden  # embedding size of first 2 SAGE convolutions
        dim_embedding = dim_embedding  # embedding size of 3rd SAGE convolutions (eq. 5, dim of Z)
        dim_embedding_MLP = dim_embedding_MLP  # hidden neurons of last 2 MLP layers
