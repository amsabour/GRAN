...
def get_loss_accuracy(loader, model):
    ...
    for data in loader:
        ...
        for data in datas:
            ...
            predictions = loss_fun._get_correct(output)

            for i in range(data.y.shape[0]):
                graph_label = data.y[i].item()
                prediction = predictions[i].item()

                if graph_label == prediction:
                    corrects[graph_label] = corrects.get(graph_label, 0) + 1

                graphs[graph_label] = graphs.get(graph_label, 0) + 1

            # corrects += accuracy * data.num_graphs / 100
            # graphs += data.num_graphs

    acc = {}
    for graph_label in graphs.keys():
        acc[graph_label] = corrects[graph_label] / graphs[graph_label]

    print(corrects, graphs)
